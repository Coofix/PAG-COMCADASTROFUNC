<?php
    ...
    $sql_code = "SELECT * FROM ENFERMIDADE_PACIENTE WHERE id_paciente = $id_paciente";
    $sql_query = $mysqli->query($sql_code) or die($mysqli->error);
    $enfermidade = $sql_query->fetch_assoc();


    $sql_code_rest = "SELECT * FROM RESTRICAO_PACIENTE WHERE id_paciente = $id_paciente";
    $sql_query_rest = $mysqli->query($sql_code_rest) or die($mysqli->error);
    $restricao = $sql_query_rest->fetch_assoc();

    $sql_code_reme = "SELECT * FROM REMEDIO_PACIENTE WHERE id_paciente = '$id_paciente'";
    $sql_query_reme = $mysqli->query($sql_code_reme) or die($mysqli->error);
    $remedio = $sql_query_reme->fetch_assoc();

    $sql_code_select = "SELECT max(id_enfermidade) as id_enfermidade FROM ENFERMIDADE";
    $sql_query_select = $mysqli->query($sql_code_select) or die($mysqli->error);
    $id = $sql_query_select->fetch_assoc();
    $id_enfermidade = $id['id_enfermidade'];
        ...
        if(count($erro) == 0){
            ...
            $sql_code_incluir = "INSERT INTO ENFERMIDADE (
                ...
                '$_SESSION[enfermidade]')";
                    ...
                    $sql_code_incluir = "INSERT INTO ENFERMIDADE_PACIENTE ( 
                        id_enfermidade,
                        id_paciente)
                        VALUES(
                        '$id_enfermidade',
                    ...
                    echo "<script> location.href='cadastropaciente3.php?id_paciente=$id_paciente';</script>";
        ...
        if(count($erro) == 0){
            ...
            $sql_code_incluir = "INSERT INTO RESTRICAO (
                ...
                if ($confirma) {
                    ...
                    $sql_code_incluir = "INSERT INTO RESTRICAO_PACIENTE ( 
                        id_restricao,
                        id_paciente)
                        VALUES(
                        $id_enfermidade,
  ...
  if(isset($_POST['incluir_remedio'])){
    ...
    // 2 - Validação dos dados
    if(strlen($_SESSION['nome_medicacao'])== 0 )
        $erro[] = "Prencha o nome da medicação";

    if(strlen($_SESSION['nome_dose'])== 0 )
        $erro[] = "Prencha a dose";

    if(strlen($_SESSION['nome_intervalo'])== 0 )
        $erro[] = "Prencha o intervalo";

    // 3 Inserção no banco de dados
    if(count($erro) == 0){

        $sql_code_incluir = "INSERT INTO REMEDIO_PACIENTE (
            nome_remedio,
            dose_remedio,
            intervalo_remedio,
            observacao_remedio,
                            ...
                            <div class="col-md-3 mb-1">
                                <table class="table table-hover">
                                    ...
                                    <?php
                                      ...
                                      ?>
                                         <tr>
                                              <td><?php echo $enfermidade['enfermidade']; ?></td>
                                          </tr>
